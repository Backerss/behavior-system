    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($user->users_role === 'student') {
                ...
                $rules['students_status'] = 'nullable|in:active,suspended,expelled,graduate';
    ...
    public function export(Request $request)
    {
        try {
            ...
            if ($request->has('search') && $request->search !== '') {
                $search = $request->search;
                $query->where(function($q) use ($search) {
                    $q->where(DB::raw("CONCAT(users_first_name, ' ', users_last_name)"), 'LIKE', "%{$search}%")
                      ->orWhere('users_email', 'LIKE', "%{$search}%");
                });
            }
            ...
            // Create CSV content
            $csvHeader = [
                'ลำดับ', 'ชื่อ-นามสกุล', 'อีเมล', 'โทรศัพท์', 'บทบาท', 'สถานะ', 
                'รหัส', 'ชั้นเรียน/แผนก', 'วันที่สมัคร'
            ];
            ...
            foreach ($users as $index => $user) {
                ...
                $csvData[] = [
                    ...
                    $user->users_email ?? '-',
                    $user->users_phone_number ?? '-',
                    $user->users_role,
                    $user->users_status,
                    $code,
                    $classOrDept,
                    $user->users_created_at ? $user->users_created_at->format('Y-m-d H:i:s') : '-'
                ];
            }
